...
class IndicatorTrader(Strategy):
    ...
    def get_closing_prices(self, symbol):
        end_date, start_date = self.get_BB_daterange()

        bars = self.api.get_bars(symbol, self.tradeapi.TimeFrame.Day, start=start_date, end=end_date).df

        # Extract closing prices
        closing_prices = bars['close']

        print(f"Closing prices for {symbol} from {start_date} to {end_date}:")
    ...
    def get_BB_buy_signal(self):
        closing_prices = self.get_closing_prices(self.symbol)
        ...
        if ((closing_prices.iloc[-1] >= lower_band) & (closing_prices.iloc[-2] < lower_band) & self.last_trade == "sell"):
            return 'buy'
        # Price breaking upper band and will experience downard pressure eventually
        elif ((closing_prices.iloc[-1] < upper_band) & (closing_prices.iloc[-2] >= upper_band) & self.last_trade == "buy"):
            return 'sell'
        else:
            return 'hold' 
